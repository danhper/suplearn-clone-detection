...
from suplearn_clone_detection.layers import SplitInput, abs_diff, DenseMulti, \
    euclidean_similarity, cosine_similarity, ModelWrapper, make_triplet_loss


def make_embeddings(lang_config: LanguageConfig, suffix: str):
    embedding_input_size = lang_config.vocabulary_size + lang_config.vocabulary_offset
    kwargs = dict(
        name="embedding_{0}_{1}".format(lang_config.name, suffix),
        mask_zero=True
    )
...
def create_lstm(output_dimension: int,
    ...
    lstm = LSTM(output_dimension,
                ...
                name="lstm_{0}_{1}_{2}".format(lang_config.name, suffix, position))

    if transformer.split_input:
        lstm = SplitInput(
            lstm, name="bidfs_lstm_{0}_{1}_{2}".format(lang_config.name, suffix, position))
    ...
    if lang_config.bidirectional_encoding:
        ...
        lstm = Bidirectional(
            lstm, name="bilstm_{0}_{1}_{2}".format(lang_config.name, suffix, position))

    return lstm


def create_input(lang_config: LanguageConfig, suffix: str):
    return Input(shape=(None,),
                 dtype="int32", name="input_{0}_{1}".format(lang_config.name, suffix))


def create_encoder(lang_config: LanguageConfig, ast_input: Input, suffix: str):
    transformer = ast_transformer.create(lang_config)

    x = make_embeddings(lang_config, suffix)(ast_input)

    for i, n in enumerate(lang_config.output_dimensions[:-1]):
        x = create_lstm(n, lang_config, transformer,
                        suffix=suffix, position=i + 1, return_sequences=True)(x)
    ...
    for i, dim in enumerate(lang_config.hash_dims):
        ...
        x = Dense(dim, use_bias=not is_last, activation=activation,
                  name="dense_{0}_{1}_{2}".format(lang_config.name, suffix, i))(x)
        if is_last:
            output_dimension = dim

    encoder = Model(inputs=ast_input, outputs=x,
                    name="encoder_{0}_{1}".format(lang_config.name, suffix))
    encoder.output_dimension = output_dimension
    return encoder

def make_merge_model(model_config: ModelConfig, input_lang1, input_lang2):
    if model_config.merge_mode == "simple" or model_config.merge_mode == "concat":
    ...
    model = Model(inputs=[input_lang1, input_lang2], outputs=x, name="merge_model")
...
def create_merge_input(lang_config: LanguageConfig,
    ...
    return Input(shape=(input_dimension,),
                 name="encoded_{0}_{1}".format(lang_config.name, suffix))


def create_optimizer(model_config: ModelConfig) -> optimizers.Optimizer:
    optimizer_class = getattr(optimizers, model_config.optimizer["type"])
    return optimizer_class(**model_config.optimizer.get("options", {}))


def create_siamese_model(model_config: ModelConfig):
    lang_config = model_config.languages[0]

    anchor_input = create_input(lang_config, "anchor")
    positive_input = create_input(lang_config, "positive")
    negative_input = create_input(lang_config, "negative")

    encoder = create_encoder(lang_config, anchor_input, "1")

    anchor_output = encoder(anchor_input)
    positive_output = encoder(positive_input)
    negative_output = encoder(negative_input)

    merged_output = concatenate([anchor_output, positive_output, negative_output])

    loss_function = make_triplet_loss(encoder.output_dimension)
    optimizer = create_optimizer(model_config)

    model = ModelWrapper(inputs=[anchor_input, positive_input, negative_input],
                         outputs=merged_output)
    model.compile(optimizer=optimizer, loss=loss_function, metrics=model_config.metrics)
...
def create_model(model_config: ModelConfig):
    ...
    input_lang1 = create_input(lang1_config, "1")
    encoder_lang1 = create_encoder(lang1_config, input_lang1, "1")
    input_lang2 = create_input(lang2_config, "1")
    encoder_lang2 = create_encoder(lang2_config, input_lang2, "2")
